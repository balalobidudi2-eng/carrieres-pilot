...
export async function POST(req: NextRequest) {
  ...
  if (!tokenRecord || tokenRecord.expiresAt < new Date()) {
    return NextResponse.json({ error: 'Session expirée' }, { status: 401, headers: CORS_HEADERS });
  }
  ...
  if (!Array.isArray(cookies) || cookies.length === 0) {
    return NextResponse.json({ error: 'cookies manquants ou vides' }, { status: 400, headers: CORS_HEADERS });
  }
  ...
  if (!automationUrl || !automationSecret) {
    return NextResponse.json({ error: 'Service d\'automatisation non configuré' }, { status: 503, headers: CORS_HEADERS });
  }
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ error: 'Erreur transmission vers le service automation' }, { status: 502, headers: CORS_HEADERS });
  }
